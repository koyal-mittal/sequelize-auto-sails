...
AutoSequelize.prototype.build = function(callback) {
  ...
  function processTables(__tables) {
    console.log(__tables,"__tables")
    if (self.sequelize.options.dialect === 'mssql')
      __tables = _.map(__tables, 'tableName');

    var tables;
    
// console.log(self.options.tables,"self.options.tables");
// console.log(self.options.skipTables,"self.options.skipTables");
    ...
    //else                              tables = __tables //for every table
  else{
    tables = _.filter(__tables,function(name){
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            ...
            if (_attr === "boolean" || _attr === "bit(1)" || _attr === "bit") {
              ...
              if(length[0]== '(36)'){
                val = 'Sequelize.UUID, \n';
                val+= spaces+spaces+spaces+'defaultValue: Sequelize.UUIDV4'
              }
              else{
            ...
            else if (_attr.match(/^(float8|double precision|numeric|double)/)) {
